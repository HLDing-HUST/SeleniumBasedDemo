...
class BaseCase(unittest.TestCase):
    ...
    def multiple_self(
            self, selectors_list, Value=None, by=By.CSS_SELECTOR, timeout=None, spacing=0
    ):
        self.__check_scope()
    ...
    def single_self(
            self, selector, Value=None, by=By.CSS_SELECTOR, timeout=None
    ):
        self.__check_scope()
        if Value == '1':
            self.click(selector, by=by, timeout=timeout)
    ...
    def __insert_test_result(self, state, err):
        ...
        if err:
            ...
            if (
                ...
                and (sb_config._results[test_id] == "跳过")
            ):
                if self.__skip_reason:
                    data_payload.message = "跳过:   " + self.__skip_reason
                else:
                    data_payload.message = "跳过:   (no reason given)"
    ...
    def __process_dashboard(self, has_exception, init=False):
        ...
        if len(sb_config._extra_dash_entries) > 0:
            ...
            for test_id in sb_config._extra_dash_entries:
                if test_id in sb_config._results.keys():
                    if sb_config._results[test_id] == "跳过":
                        sb_config.item_count_skipped += 1
                        sb_config.item_count_untested -= 1
                    elif sb_config._results[test_id] == "失败":
                        sb_config.item_count_failed += 1
                        sb_config.item_count_untested -= 1
                    elif sb_config._results[test_id] == "完成":
        ...
        if not init:
            ...
            if test_id in sb_config._results.keys() and (
                ...
                if test_id not in sb_config._results.keys():
                    sb_config._results[test_id] = "失败"
                    sb_config.item_count_failed += 1
                    sb_config.item_count_untested -= 1
                elif not sb_config._results[test_id] == "失败":
                    # tearDown() was called more than once in the test
                    if sb_config._results[test_id] == "完成":
                ...
                if (
                    test_id in sb_config._results.keys()
                    and sb_config._results[test_id] == "失败"
                ):
        ...
        self.add_data_point("完成", num_passed, color="#84d474")
        self.add_data_point("未执行", num_untested, color="#eaeaea")
        self.add_data_point("跳过", num_skipped, color="#efd8b4")
        self.add_data_point("失败", num_failed, color="#f17476")
        style = (
        ...
        table_html = (
            ...
            '<th col="result">结果</th><th col="name">RPA任务名称</th>'
            '<th col="duration">耗时</th><th col="links">链接</th>'
            "</tr></thead>"
        )
        the_failed = []
        the_skipped = []
        ...
        for row in the_failed:
            row = (
                ...
                '<td><a href="%s">日志</a> / <a href="%s/">截图</a>'
                "</td></tr></tbody>"
        ...
        for row in the_untested:
            ...
            table_html += row
        table_html += "</table>"
        add_more = "<br /><b>最后更新时间:</b> "
        timestamp, the_date, the_time = log_helper.get_master_time()
        last_updated = "%s at %s" % (the_date, the_time)
        add_more = add_more + "%s" % last_updated
        status = "<p></p><div><b>状态:</b> 等待结果..."
        status += " (刷新界面查看最新进展)"
        if num_untested == 0:
            status = "<p></p><div><b>状态:</b> 所有RPA任务均已执行完毕:"
            if num_failed == 0:
                if num_passed > 0:
                    if num_skipped == 0:
                        status += " <b>完成!</b> (所有RPA任务均已成功完成)"
                    else:
                        status += " <b>完成!</b> (没有失败的RPA任务)"
                else:
                    status += " 所有的测试已跳过!"
            else:
                latest_logs_dir = "最新日志/"
                log_msg = "在最新的日志文件中查看更多细节"
                if num_failed == 1:
                    status += (
                        " <b>1 个RPA任务执行失败!</b> --- "
                        '(<b><a href="%s">%s</a></b>)'
